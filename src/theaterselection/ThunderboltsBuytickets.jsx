...
//                 time: showTime,
//                 screen: screen,
//                 movieName: "Thunderbolts",  // Assuming "Thunderbolts" is static for this page
...
//     const layoutPath = `/thunderbolts/buytickets/${formattedTheater}seatlayout`;
...
function ThunderboltsBuytickets() {
  ...
  // ✅ Fetch theaters for "Thunderbolts" movie
  useEffect(() => {
    const fetchTheaters = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/movies");
        const thunderMovie = response.data.find(
          (m) => m.name.toLowerCase() === "thunderbolts"
        );

        if (!thunderMovie) {
          console.warn("Thunderbolts movie not found in response");
          return;
        }

        // ✅ Map enabled theaters with proper nested screen + showtime structure
        const enabledTheaters = thunderMovie.theaters
