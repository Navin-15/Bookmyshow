...
// RetroBuytickets.jsx  previous hardcoded code  
...
// 15/10/2025 working dynamicaly  fetching from admin

//dynamically controlled the theaters from admin panel 
...
//             const enabledTheaters = response.data.filter(theater => theater.isEnabled === true);
...
//                     theater.layoutPath || "/default-layout"
//                   )
//                 }
//               >
//                 <div className="movie-time mt-2">{time}</div>
//                 <sub className="mt-4">{theater.screen?.[i] || "Screen"}</sub>
//               </div>
//             </div>
//           ))}
//         </div>
...
//         const retroMovie = response.data.find((m) => m.name.toLowerCase() === "retro");
//         // const enabledTheaters = retroMovie.theaters.filter((t) => t.isEnabled);
//         // setTheaters(enabledTheaters);
//         const enabledTheaters = retroMovie.theaters.filter((t) => t.isEnabled).map((t) => ({
//         cinema: t.name, // ✅ rename to match frontend usage
//         showTime: t.screens.flatMap((s) => s.showTimes), // ✅ collect all showtimes
//         screen: t.screens.map((s) => s.screenName), // ✅ collect all screen names
//   }));
...
//     const layoutPath = `/retro/buytickets/${formattedTheater}seatlayout`;
...
function RetroBuytickets() {
  ...
  useEffect(() => {
    const fetchTheaters = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/movies");
        const retroMovie = response.data.find(
          (m) => m.name.toLowerCase() === "retro"
        );

        // ✅ Map enabled theaters with proper nested screen + showtime structure
        const enabledTheaters = retroMovie.theaters
